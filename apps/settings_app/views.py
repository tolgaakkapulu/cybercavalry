...
SETTING_LABELS = {
    ...
    'actions.silence_alert_enabled':                ('Enable Silence Alert',      'Send an e-mail when the platform receives no API traffic for the configured window. Detects the whole integration surface going quiet (firewall / XDR / SIEM cron dead, revoked token, network path change).'),
    'actions.silence_alert_email':                  ('Recipient E-mail',          'Address(es) that receive silence alerts. Separate multiple with `;`.'),
    'actions.silence_track_get':                    ('Track GET requests',        'Count GET-shaped list endpoints (blacklist / hashlist / urllist / status) as API activity. When enabled, a GET request keeps the silence timer alive.'),
    'actions.silence_track_post':                   ('Track POST requests',       'Count POST-shaped report endpoints (IP / hash / URL report) as API activity. When enabled, a POST request keeps the silence timer alive.'),
    'actions.silence_threshold_minutes':            ('Silence Threshold',         'Fire the alert when zero tracked API requests have arrived in this many minutes. Default 5.'),
    'actions.silence_alert_cooldown_hours':         ('Alert Cooldown',            'After an alert is sent, suppress further silence e-mails for this many hours. Default 6.'),
    # Syslog forwarding
...
def settings_index(request):
    ...
    _ENSURE = {
        ...
        'general.default_theme':       ('dark',     'str', 'general',  'Default theme for users without a saved preference (light/dark)', False),
        ...
        'actions.silence_alert_enabled':          ('false', 'bool', 'actions', 'Send alert e-mails when the platform receives no tracked API activity for the silence window', False),
        'actions.silence_alert_email':            ('',      'str',  'actions', 'Recipient e-mail address for silence alerts', False),
        'actions.silence_track_get':              ('true',  'bool', 'actions', 'Count GET list endpoints (blacklist/hashlist/urllist/status) as traffic', False),
        'actions.silence_track_post':             ('true',  'bool', 'actions', 'Count POST report endpoints (IP/hash/URL report) as traffic', False),
        'actions.silence_threshold_minutes':      ('5',     'int',  'actions', 'Fire the alert when no tracked API request has arrived in this many minutes', False),
        'actions.silence_alert_cooldown_hours':   ('6',     'int',  'actions', 'Suppress repeat silence alerts for this many hours after one fires', False),
    ...
    for cat, items in settings_by_category.items():
        ...
        _KEY_ORDER = {
            ...
            'actions.silence_track_get':                 27,
            'actions.silence_track_post':                28,
            'actions.silence_threshold_minutes':         29,
            'actions.silence_alert_cooldown_hours':      30,
