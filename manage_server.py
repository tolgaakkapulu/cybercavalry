...
def cmd_clean(args):
    ...
    # 1 - Python cache
    log("[1/8] Removing Python cache files...", CYAN)
    ...
    # 2 - Veritabani
    log("[2/8] Removing database...", CYAN)
    remove(BASE_DIR / "cybercavalry.db", "cybercavalry.db")

    # 3 - Dosya cache
    log("[3/8] Clearing file cache...", CYAN)
    remove(BASE_DIR / ".cache", ".cache/")

    # 4 - Static dosyalar
    log("[4/8] Removing collected staticfiles...", CYAN)
    remove(BASE_DIR / "staticfiles", "staticfiles/")

    # 5 - Log dosyalari
    log("[5/8] Removing log files...", CYAN)
    removed_logs = False
    ...
    if args.full:
        log("[6/8] Removing SSL certificates (--full)...", CYAN)
        ...
        log("[6/8] Keeping SSL certificates (use --full to also remove them)", YELLOW)

    # 7 - Offline wheel bundle (opsiyonel)
    # `deploy/wheels/` + `deploy/wheels.py*.lock.txt` are install-time
    # artifacts generated by `prepare_offline_bundle.py`. Kept during a
    # normal clean (rebuilds the venv would still need them if PyPI is
    # unreachable), removed under --full to fully reset the tree.
    if args.full:
        log("[7/8] Removing offline wheel bundle (--full)...", CYAN)
        wheels_dir = BASE_DIR / "deploy" / "wheels"
        if wheels_dir.exists():
            shutil.rmtree(wheels_dir, ignore_errors=True)
            log(f"  {OK} Removed: deploy/wheels/", GREEN)
        else:
            log(f"  {SKIP} deploy/wheels/ not found.", YELLOW)
        removed_locks = False
        for lock in (BASE_DIR / "deploy").glob("wheels.py*.lock.txt"):
            lock.unlink(missing_ok=True)
            log(f"  {OK} Removed: {lock.relative_to(BASE_DIR)}", GREEN)
            removed_locks = True
        if not removed_locks:
            log(f"  {SKIP} No wheels lock files found.", YELLOW)
    else:
        log("[7/8] Keeping offline wheel bundle (use --full to also remove it)", YELLOW)

    # 8 - venv (opsiyonel)
    if args.full:
        log("[8/8] Removing virtual environment (--full)...", CYAN)
        ...
        log("[8/8] Keeping venv (use --full to also remove it)", YELLOW)
